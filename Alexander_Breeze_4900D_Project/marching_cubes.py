...
def process_cube(cube_point, cube_val, useOccupancy=False):
...
def marching_cubes(start, end, num_cubes_per_dim, fun, merge_dup=True, useOccupancy=False):
    ...
    # Precompute the value at each cube vertex to cut down duplicate calculations
    value_lattice = np.zeros((num_cubes_per_dim + 1, num_cubes_per_dim + 1, num_cubes_per_dim + 1), dtype=float)
    zi = 0
    z = start[2]
    while z <= end[2]:
        #print(z)
        ...
        while y <= end[1]:
            ...
            x = start[0]
            while x <= end[0]:
                value_lattice[xi, yi, zi] = fun(np.array([x, y, z]))
                xi += 1
                x += cube_size[0]
            yi += 1 
            y += cube_size[1]
    ...
    cube_point = np.zeros((8, 3), dtype=float)
    cube_val = np.zeros((8), dtype=float)
            ...
            while x < end[0]:
                ...
                for k in range(2):
                    ...
                    for shift in [[0, 0], [1, 0], [1, 1], [0, 1]]:
                        ...
                        # corner
                        cube_val[point_index] = value_lattice[xi + shift[0], yi + shift[1], zi + k]
